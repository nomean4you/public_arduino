...
void fetchAndDisplayTickerInfo(const String& ticker) {
  ...
  if (httpCode == 200) {  // 요청이 성공했다면
    ...
    if (selectedInfo.indexOf("change") != -1 && doc[0]["change"] != nullptr) {
      tft.setCursor(0, cursorY);
      tft.println("Change: " + String(doc[0]["change"].as<const char*>()));  // 문자열로 안전하게 처리
      cursorY += 20;
    }
    if (selectedInfo.indexOf("change_price") != -1 && doc[0]["change_price"] != nullptr) {
      tft.setCursor(0, cursorY);
      tft.println("Cha. Price: " + formatPrice(doc[0]["change_price"].as<float>()));
      cursorY += 20;
    }
    if (selectedInfo.indexOf("change_rate") != -1 && doc[0]["change_rate"] != nullptr) {
      tft.setCursor(0, cursorY);
      tft.println("Cha. Rate: " + String(doc[0]["change_rate"].as<float>()) + "%");
      cursorY += 20;
    }
